	...
	$(document).ready(function(){
		...
		$('#SubmitOrderBtn').on('click', function(e){
			...
			if(nume.length >=3  && prenume.length >= 3 && phoneTrue){
				if(eC == 0){
					alert_danger("Nu este nici un element adaugat in cos");
				}
				else{
				...
				$.ajax({
					...
					error: function(data){
						alert_danger("Probleme cu partea de server");
					}
				});
			}
		}
		else{
			alert_danger("Datele Introduse sunt incorecte");
		}
	});
	...
	function addBucket(n,c){
		...
		alert_success("Produsul a fost adaugat cu succes");
	}
	var sum = 0;
